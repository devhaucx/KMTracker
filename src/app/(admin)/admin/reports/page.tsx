...
export default function AdminReportsPage() {
  ...
  const exportCSV = () => {
    ...
    const deptRows = MOCK_DEPARTMENT_LEADERBOARD.map(dept => [
      dept.overall_rank,
    ...
    const indRows = MOCK_INDIVIDUAL_LEADERBOARD.map(ind => [
      ind.overall_rank,
      ind.full_name,
      ind.department_name,
      ind.sport_type,
    ...
    exportToCSV(`Bao_Cao_Biet_Doi_${MOCK_COMPETITION.invite_code}_2026.csv`, indHeaders, indRows)
    setTimeout(() => setDownloading(false), 500)
  }

  // Top runner per sport
  const getTopRunnerBySport = (sport: SportType) => {
    return MOCK_INDIVIDUAL_LEADERBOARD.find((i) => i.sport_type === sport)
  }

  const topRun = getTopRunnerBySport('Run')
  const topRide = getTopRunnerBySport('Ride')
  const topSwim = getTopRunnerBySport('Swim')
  const topWalk = getTopRunnerBySport('Walk')
    ...
    <div className="container" style={{ padding: '2rem 1.5rem 4rem' }}>
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '0.2rem' }}>
            {MOCK_COMPETITION.name} · Dữ liệu tự động cập nhật từ Strava
          </p>
        </div>

        <button onClick={exportCSV} disabled={downloading} className="btn btn-primary" style={{ gap: '0.4rem' }}>
      ...
      >
        <div className="stat-card">
          ...
          <div className="stat-value">{MOCK_ADMIN_STATS.participant_count}</div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: 'var(--color-primary)' }}>
            {MOCK_ADMIN_STATS.total_converted_km}
          </div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {MOCK_DEPARTMENT_LEADERBOARD.map((dept) => {
      ...
      <div>
        ...
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {MOCK_INDIVIDUAL_LEADERBOARD.slice(0, 5).map((ind) => (
                <tr key={ind.user_id}>
                  <td style={{ textAlign: 'center', fontWeight: 700 }}>
                    {ind.overall_rank === 1
                      ? '🥇 #1'
